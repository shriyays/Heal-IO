  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 7, marginBottom: 20 }}>
        {['7d', '30d', '90d'].map((r) => (
          <button key={r} type="button" className={`rpill${range === r ? ' on' : ''}`} onClick={() => setRange(r)}>
            {r}
          </button>
        ))}
      </div>
      ...
      <div className="g2" style={{ marginBottom: 12 }}>
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
            <div className="sec-lbl" style={{ margin: 0 }}>Trends — {range}</div>
            <div style={{ display: 'flex', gap: 12 }}>
              {[['Mood', '#0a6e5c'], ['Energy', '#059669'], ['Pain', '#dc2626']].map(([lbl, color]) => (
                <div key={lbl} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10.5, color: '#6a9a8a' }}>
          ...
          {filtered.length === 0 ? (
            <p style={{ fontSize: 12, color: '#6a9a8a' }}>No data for this period. Start logging to see trends.</p>
          ) : (
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 90 }}>
              {chartLogs.map((l) => (
                <div key={l._id} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                  <div style={{ display: 'flex', gap: 2, alignItems: 'flex-end', height: 70 }}>
                    {[[l.mood || 0, '#0a6e5c'], [l.energy || 0, '#059669'], [l.pain || 0, '#dc2626']].map(([val, color], j) => (
                      <div key={j} style={{ width: 7, height: `${(val / 10) * 70}px`, background: color, opacity: 0.75 }} />
                    ))}
                  </div>
        ...
        <div className="card">
          ...
          {[['Poor (&lt;6h)', poorPain, '#dc2626'], ['Okay (6–7h)', okayPain, '#d97706'], ['Good (7h+)', goodPain, '#059669']].map(
            ([lbl, val, color]) => (
          ...
          {goodPain !== '—' && poorPain !== '—' && (
            <div className="insight">
              ...
              <strong>{Math.round((1 - parseFloat(goodPain) / parseFloat(poorPain)) * 100)}%</strong>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="card">
          ...
          {topSyms.length === 0 ? (
            ...
            topSyms.map(([sym, count], idx) => (
              <div key={sym} style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 11 }}>
                <div style={{ width: 6, height: 6, background: SYM_COLORS[idx], flexShrink: 0 }} aria-hidden="true" />
                <div style={{ flex: 1, fontSize: 12.5, color: '#2d4a3e' }}>{sym}</div>
                <span style={{ fontSize: 11.5, fontWeight: 700, color: SYM_COLORS[idx] }}>{count}x</span>
                <div style={{ width: 80 }}>
            ...
            {radarItems.map(([lbl, val, color]) => (
              <div key={lbl} style={{ textAlign: 'center' }}>
                <div style={{ position: 'relative', width: 44, height: 44, margin: '0 auto 5px' }}>
                  <svg viewBox="0 0 36 36" style={{ transform: 'rotate(-90deg)', width: 44, height: 44 }} aria-hidden="true">
                    <circle cx="18" cy="18" r="14" fill="none" stroke="rgba(10,110,92,0.12)" strokeWidth="3" />
                    <circle cx="18" cy="18" r="14" fill="none" stroke={color} strokeWidth="3"
                      strokeDasharray={`${(val / 10) * 88} 88`} strokeLinecap="butt" />
                  </svg>
                  <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', fontSize: 10.5, fontWeight: 700, color }}>
                    {val}
                  </div>
                </div>
                <div style={{ fontSize: 9.5, color: '#6a9a8a', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{lbl}</div>
              </div>
            ))}
          </div>
        </div>
